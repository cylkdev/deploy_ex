defmodule Mix.Tasks.DeployEx do
  ...
  @tasks [
    # DeployEx Commands
    {"deploy_ex.full_setup",
     "Performs complete infrastructure and application setup using Terraform and Ansible"},
    {"deploy_ex.full_drop",
     "Completely removes DeployEx configuration and files from the project"},
    {"deploy_ex.install_github_action",
     "Installs a GitHub Action for automated infrastructure and deployment management"},
    {"deploy_ex.install_migration_script",
     "Installs a migration script for managing database migrations during deployment"},
    ...
    {"deploy_ex.ssh.authorize",
     "Add or remove ssh authorization to the internal network for specific IPs"},
    {"deploy_ex.download_file", "Downloads a file from a remote server using SCP"},
    {"deploy_ex.list_app_release_history",
     "Lists the latest releases for a specific app via SSH"},
    {"deploy_ex.list_available_releases",
     "Lists all available releases uploaded to the release bucket"},
    {"deploy_ex.view_current_release",
     "Shows the current (latest) release for a specific app via SSH"},
    {"deploy_ex.test", "Runs mix.release for apps that have changed"},

    # Autoscaling Commands
    {"deploy_ex.autoscale.status",
     "Displays autoscaling group status (instance count, limits, policies)"},
